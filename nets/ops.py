...
def res_block(input, expansion_ratio, output_dim, stride, is_train, name, bias=False, shortcut=True, end_points=None):
    with tf.name_scope(name), tf.variable_scope(name):
        # pw
...
def separable_conv(input, k_size, output_dim, stride, pad='SAME', channel_multiplier=1, name='sep_conv', bias=False, data_format="NHWC"):
    with tf.name_scope(name), tf.variable_scope(name):
        in_channel = input.get_shape().as_list()[-1]
        ...
        conv=tf.nn.separable_conv2d(input, dwise_filter, pwise_filter, strides, padding=pad, name=name, data_format=data_format)
        if bias:
            biases = tf.get_variable('bias', [output_dim],initializer=tf.constant_initializer(0.0))
            conv = tf.nn.bias_add(conv, biases)
        return conv
